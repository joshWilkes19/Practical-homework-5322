...
#using bagging
cig.bag <- randomForest(TOBFLAG ~ ., data = cig.train_clean[, c("TOBFLAG", demographic_cols)], mtry = length(demographic_cols), importance = TRUE, ntree = 80)
...
cig.rf <- randomForest(TOBFLAG ~ ., data = cig.train_clean[, c("TOBFLAG", demographic_cols)], mtry = 3, ntree = 100, importance = TRUE)
...
cig.rf <- randomForest(CIGUSE_LEVEL ~ ., data = cig.train_clean[, c("CIGUSE_LEVEL", demographic_cols)], mtry = 3, ntree = 100, importance = TRUE)
...
set.seed(123)
rf_model <- randomForest(tree_formula, data = train_data, ntree = 100, importance = TRUE)
...
# Plot performance (cross-validated error)
plot(cig_boost)
